...
public class UrunManager implements UrunService {
	...
	@Autowired
	public UrunManager(UrunDao urunDao, ModelMapperService modelMapperService, SaticiDao saticiDao, MarkaDao markaDao,
			KategoriDao kategoriDao, FiyatDao fiyatDao) {
	...
	public DataResult<Urunler> getByUrunAdiAndKategoriId(String urunAdi, int kategoriId) {
		return new SuccessDataResult<Urunler>(this.urunDao.getByUrunAdiAndKategoriId(urunAdi, kategoriId),
				"Data listelendi.");
	}

	@Override
	public DataResult<List<Urunler>> getByUrunAdiOrKategoriId(String urunAdi, int kategoriId) {
		return new SuccessDataResult<List<Urunler>>(this.urunDao.getByUrunAdiOrKategoriId(urunAdi, kategoriId),
				"Data listelendi.");
	}
	...
	public DataResult<List<Urunler>> getBySatici_SaticiFirmaAdiIn(List<String> saticiFirmaAdi) {
		return new SuccessDataResult<List<Urunler>>(this.urunDao.getBySatici_SaticiFirmaAdiIn(saticiFirmaAdi),
				"Data listelendi.");
	}
	...
	public DataResult<Urunler> getById(int id) {
		Urunler urun = urunDao.findById(id).orElseThrow(() -> new IllegalArgumentException("Ürün bulunamadı."));
	...
	public Result addUrunForSatici(SaticiUrunKayitDto saticiUrunKayitDto) {
		...
		urun.setSatici(saticiDao.findById(saticiUrunKayitDto.getSaticiId())
				.orElseThrow(() -> new IllegalArgumentException("Satıcı bulunamadı.")));
		urun.setMarka(markaDao.findById(saticiUrunKayitDto.getMarkaId())
				.orElseThrow(() -> new IllegalArgumentException("Marka bulunamadı.")));
		urun.setKategori(kategoriDao.findById(saticiUrunKayitDto.getKategoriId())
	...
	public Result saticiUrunUpdate(SaticiUrunUpdateDto saticiUrunUpdateDto, int saticiId, int urunId) {
		...
		if (!urun.getSatici().equals(existingSatici)) {
			throw new IllegalArgumentException("Bu ürün bu satıcıya ait değil.");
		}
		urun.setStok_Sayisi(saticiUrunUpdateDto.getStokSayisi());

		Fiyat yeniFiyat = new Fiyat();
		yeniFiyat.setUrun(urun);
		yeniFiyat.setUrun_Fiyat(saticiUrunUpdateDto.getFiyat());
		urun.getFiyatlar().add(0, yeniFiyat);
