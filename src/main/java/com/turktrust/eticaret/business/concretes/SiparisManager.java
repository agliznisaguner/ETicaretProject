...
public class SiparisManager implements SiparisService {
	...
	@Autowired
	public SiparisManager(SiparisDao siparisDao, ModelMapperService modelMapperService, UrunDao urundao,
			SepetDao sepetDao, AdresDao adresDao) {
		super();
	...
	public Result siparisOlustur(SiparisOlusturDto siparisOlusturDto) {
		...
		double toplamFiyat = urunler.stream().mapToInt(urun -> urun.getFiyatlar().get(0).getUrun_Fiyat()).sum();
		...
		for (Urunler urun : urunler) {
			if (urun.getStok_Sayisi() <= 0)
				return new ErrorResult("Ürün stoğu yetersiz. Lütfen ürünü sepetten çıkardıktan sonra tekrar deneyin.");
